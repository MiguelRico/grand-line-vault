function ArjunkaiOptcgProvider() { this.client = new ArjunkaiOptcgClient(); }
ArjunkaiOptcgProvider.prototype.getProviderInfo = function () {
  return { id: 'ARJUNKAI_OPTCG', name: 'Arjunkai OPTCG', baseUrl: Config.get('ARJUNKAI_API_BASE_URL'), documentationUrl: 'https://github.com/arjunkai/optcg-api' };
};
ArjunkaiOptcgProvider.prototype.getCapabilities = function () { return ProviderCapabilities.ARJUNKAI_OPTCG; };
ArjunkaiOptcgProvider.prototype.isEnabled = function () { return Config.bool('ARJUNKAI_ENABLED'); };
ArjunkaiOptcgProvider.prototype.isConfigured = function () {
  return Boolean(Config.get('ARJUNKAI_API_BASE_URL')) &&
...
ArjunkaiOptcgProvider.prototype.search = function (criteria) {
  var response = this.client.get('/cards', {
    ...
    variant_type: String(criteria.variant || '').toLowerCase(),
    min_cost: criteria.minCost, max_cost: criteria.maxCost,
    min_power: criteria.minPower, max_power: criteria.maxPower,
    min_price: criteria.minPrice, max_price: criteria.maxPrice,
    sort: criteria.sort, order: criteria.direction,
    page: criteria.page, page_size: criteria.pageSize
  });
  ...
  return {
    items: cards, page: criteria.page, pageSize: criteria.pageSize,
