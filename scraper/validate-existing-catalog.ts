...
async function main(): Promise<void> {
  ...
  if (manifest.schemaVersion !== SCHEMA_VERSION)
    throw new Error('Schema del manifest incompatible.');
  const resolvePublic = (url: string) =>
  ...
  for (const payload of [cardsPayload, setsPayload]) {
    ...
    if (payload.catalogVersion !== manifest.catalogVersion)
      throw new Error('Versiones inconsistentes.');
  }
  ...
  console.log(
    `Catálogo ${manifest.catalogVersion} válido: ${data.cards.length} cartas, ${data.sets.length} series.`,
  );
}
